...
class MultimirrorCrypto
{
    ...
    public static function parse_master_key($encryption_master_key, $encryption_master_key_base64)
    {
        if (!function_exists('sodium_crypto_secretbox')) {
            throw new MultimirrorException('To use end to end encryption, you must either be using PHP 7.2 or greater or have installed the libsodium-php extension for php < 7.2.');
        }

        if ($encryption_master_key != '' and $encryption_master_key_base64 != '') {
            throw new MultimirrorException('Do not specify both encryption_master_key and encryption_master_key_base64. encryption_master_key is deprecated, use only encryption_master_key_base64');
        }

        if ($encryption_master_key != '') {
            if (strlen($encryption_master_key) != SODIUM_CRYPTO_SECRETBOX_KEYBYTES) {
                throw new MultimirrorException('encryption_master_key must be 32 bytes. It is also deprecated, use encryption_master_key_base64');
            }
        ...
        if ($encryption_master_key_base64 != '') {
            ...
            if ($decoded_key === false) {
                throw new MultimirrorException('encryption_master_key_base64 must be a valid base64 string');
            }

            if (strlen($decoded_key) != SODIUM_CRYPTO_SECRETBOX_KEYBYTES) {
                throw new MultimirrorException('encryption_master_key_base64 must encode a key which is 32 bytes long');
            }
    ...
    /**
     * Initialises a MultimirrorCrypto instance.
     *
    ...
    public function decrypt_event($event)
    {
        ...
        if (!$decrypted_payload) {
            throw new MultimirrorException('Decryption of the payload failed. Wrong key?');
        }
    ...
    public function generate_shared_secret($channel)
    {
        if (!self::is_encrypted_channel($channel)) {
            throw new MultimirrorException('You must specify a channel of the form private-encrypted-* for E2E encryption. Got '.$channel);
        }
    ...
    {
        if (!self::is_encrypted_channel($channel)) {
            throw new MultimirrorException('Cannot encrypt plaintext for a channel that is not of the form private-encrypted-*. Got '.$channel);
        }
    ...
    private function parse_encrypted_message($payload)
    {
        ...
        if (strlen($decoded_payload->nonce) != SODIUM_CRYPTO_SECRETBOX_NONCEBYTES || $decoded_payload->ciphertext == '') {
            throw new MultimirrorException('Received a payload that cannot be parsed.');
        }
